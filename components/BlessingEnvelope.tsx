...
export const BlessingEnvelope: React.FC<BlessingEnvelopeProps> = ({ isOpen, name = "Jack" }) => {
  const [showContent, setShowContent] = useState(false);

  useEffect(() => {
    let timer: number;
    if (isOpen) {
      // Delay text appearance until envelope settles
  ...
  return (
    <div 
      className={`fixed inset-0 flex items-center justify-center pointer-events-none z-[100] transition-all duration-1000 ease-out
        ${isOpen ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-20 scale-95'}`}
    >
      {/* Container with Frosted Glass & Rose Gold Accents */}
      <div className="relative w-[350px] max-w-[90vw] h-[520px] bg-pink-900/10 backdrop-blur-[20px] border border-pink-300/30 rounded-2xl shadow-[0_40px_100px_rgba(255,128,160,0.15)] overflow-hidden flex flex-col">
        ...
        {/* Letter Content */}
        <div className={`flex-1 p-8 pt-44 flex flex-col items-center text-center transition-all duration-1000 transform ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div className="space-y-4 max-w-full">
            ...
            <p className="font-handwriting text-white/80 text-[1.2rem] leading-[1.4] italic px-1">
          ...
          <div className="mt-8 pt-6 border-t border-white/5 w-full">
            <p className="font-handwriting text-pink-300/80 text-[1.1rem] length-[1.4]">
              “And the peace of God, which transcends all understanding, will guard your hearts and your minds in Christ Jesus.”
            </p>
            <p className="mt-2 text-[10px] text-pink-400/50 font-black uppercase tracking-[0.4em]">
              (Philippians 4:7)
            </p>
          </div>
        </div>
